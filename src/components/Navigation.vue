            ...
            <div class="hidden md:block">
              <ul class="ml-10 flex items-baseline space-x-4">
                <li v-for="item in mainNav" :key="item.name">
                  <NavItem
                    v-if="
                      (item.meta.requiresAdmin === true &&
                        $store.state.rights === 1) ||
                      item.meta.requiresAdmin === false
                    "
                    :href="item.href"
                    :is-active="currentRoutePath === item.href ? true : false"
                  >
                    {{ item.name }}
                  </NavItem>
                </li>
                <li>
                  <NavItem
                    v-if="$store.state.username === undefined"
                    href="/login"
                    ><i18n-t keypath="login"
                  /></NavItem>
                </li>
                <li>
                  <NavItem v-if="$store.state.username" href="/login/:logout"
                    ><i18n-t keypath="logout"
                  /></NavItem>
                </li>
              </ul>
            </div>
